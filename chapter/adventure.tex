...
A haven is any safe patch of civilization where characters can take restful activity. The \textbf{unit of time} in haven is days, during which characters can take one action. Sample actions are as follows:
\begin{itemize}
...
A demesne is a territory controlled and maintained by a character. Typically, parties of level 6 and above might acquire a demesne as part of the story. The \textbf{unit of time} in demesne action is seasons, during which characters can take two actions. Sample actions are as follows:
\begin{itemize}
...
\begin{enumerate}
    ...
    \item {The roll's difficulty is set by the FOE being attacked's HIT VALUE;}
...
\begin{enumerate}
    ...
    \item {Each PLAYER character on the winning side rolls a d6, if they roll over their HIT VALUE, they take a number of hits equal to the number rolled;}
    \item {Each PLAYER character on the losing side rolls a d6, if they roll over their HIT VALUE, they are slain or captured;}
    \item {Each CONDUCTOR character on the losing side is slain, captured, or driven off.}
\end{enumerate}
...
\begin{itemize}
    ...
    \item {Five minutes before the end of session, players answer these questions as a group, marking 1 EXPERIENCE for each `Yes':}
    \begin{itemize}
    ...
    \begin{itemize}
        ...
        \item {Their level increases by 1;}
    ...
    \item {The session then ends. The group may schedule the next.}
\end{itemize}